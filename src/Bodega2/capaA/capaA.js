...
function displayPilones(pilones) {
    ...
    pilones.forEach(pilon => {
        ...
        tr.innerHTML = `
            ...
            <td>${pilon.corte}</td>
            <td>${pilon.etapa}</td>
            <td>${pilon.capa_a_seco_grande}</td>
            <td>${pilon.capa_a_seco_claro_grande}</td>
            <td>${pilon.capa_a_seco_mediana}</td>
            <td>${pilon.capa_a_viso}</td>
            <td>${pilon.total_capa_a}</td>
            <td>
                <div class="dropdown">
...
function abrirModalEditarPilon(idPilon, pilon_id) {
    const url = `../actualizarProceso/actualizarProceso.html?id=${idPilon}&pilonId${pilon_id}`;
...
async function generarReporte(pilonId) {
  try {
      ...
      if (response.ok) {
        ...
        const filas = descripciones.filter(desc => desc !== "Total Escogido")  // Filtra fuera "Total Escogido"
            .map((desc, index) => {
                const clave = claves[desc];
            ...
            let librasMojadero = pilon.libras_mojadero; // Ejemplo: 500
            let totalEscogido = pilon.total_escogido;   // Ejemplo: 1200
